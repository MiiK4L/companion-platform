...
- un **volet documentaire** (lots L1–L8) — **terminé** : analyses, comparaisons,
  protocoles rédigés, séries représentatives sourcées ;
- un **volet expérimental** (campagnes de mesure, prototypes, rapports, création
  des ADR 0013+) — **encore ouvert**.

**La Phase 1 n'est donc PAS close.** Ce Lot 9 est un **checkpoint documentaire** :
il consolide le volet documentaire et **prépare** le volet expérimental. Les
...
La préparation d'un essai n'est pas binaire. On distingue **cinq niveaux** :
...
3. **Protocole instancié** — décliné pour une **option / un DUT** précis.
4. **Protocole revu** — relu et corrigé (statut *Revu*).
5. **Baseline d'essai approuvée** — seuils `[BL]` fixés, baseline validée (statut *Baseline d'essai*).

> **Aujourd'hui, l'état général est aux niveaux 1–2.** Les niveaux 3–5 restent à
> produire. **Aucun protocole avec un champ `[BL]` ouvert n'est « prêt à
> exécuter ».**
...
| **Proto rédigé** | Le protocole **générique** est-il rédigé (statut Brouillon) ? |
...
| `DEC-*` | Doc | Proto rédigé | Baseline | Banc | Compos. | Mesurable | Verdict |
|---------|:---:|:---:|:---:|:---:|:---:|:---:|---------|
| `DEC-L1-001` | ✓ | ◑ | ○ | ◑ | ◑ | ◑ | Protocole de bring-up **à instancier** depuis le [modèle](../templates/test-protocol-template.md) ; baseline à approuver |
| `DEC-L1-002` | ✓ | ◑ | ○ | ◑ | ◑ | ○ | **Gated** : dépend des résultats L4 (RAM) et L5 (deep-sleep) ; réutilise leurs bancs |
| `DEC-L2-001` | ✓ | ✓ | ○ | ◑ | ◑ | ◑ | Protocoles génériques rédigés ; instanciation + baseline + banc d'injection à produire |
...
| `DEC-L3-002` | ✓ | ✓ | ○ | N/A | N/A | ◑ | **Essai logiciel** (parsing borné sur **corpus**) : ni banc ni composant physique ; **corpus/fuzzing à préparer**, baseline à approuver |
...
- l'**instanciation** et le **baselining** des protocoles (niveaux 3–5) ;
...
3. **Protocole instancié + baseliné** — décliné par option/DUT, seuils `[BL]`
   fixés, statut **Baseline d'essai** (cf. [cadre de validation](../validation-framework.md)) ;
