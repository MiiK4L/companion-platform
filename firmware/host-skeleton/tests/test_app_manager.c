...
void test_launch_success(void) {
  HostComposition c;
  host_composition_build(&c, "hello", &kOpaque, RT_OK);
...
// Source ne résout pas -> erreur source, runtime JAMAIS appelé.
...
void test_runtime_failure(void) {
  HostComposition c;
  host_composition_build(&c, "hello", &kOpaque, RT_ERR);
...
void test_runtime_swappable(void) {
  HostComposition ok;
  host_composition_build(&ok, "hello", &kOpaque, RT_OK);
  HostComposition ko;
  host_composition_build(&ko, "hello", &kOpaque, RT_ERR);

  // Même service (même code), moteurs bouchons différents -> résultats différents.
  EXPECT_EQ_INT(app_manager_launch(&ok.app_manager, "hello"), AM_OK,
                "swap: OK runtime -> success");
  EXPECT_EQ_INT(app_manager_launch(&ko.app_manager, "hello"), AM_ERR_RUNTIME,
